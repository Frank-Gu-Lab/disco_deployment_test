...
if "time" in st.session_state:
    ...
    if choice == "Upload and analyze (Step 1)":
        ...
        if len(list_of_raw_books) > MAX_BOOKS:
            st.warning("Warning: Max file upload limit of 6.  Only the first 6 books will be processed")
        ...
        if i == 6:
            try:
                ...
                try:
                    ...
                    for index, row in proton_summary_df.iterrows():
                        if not ((database_df["polymer_name"] == row["polymer_name"]) & (database_df["ppm"] == row["ppm"])).any():
                            database_df.append(row)

                    database_df.to_pickle("proton_binding_dataset_db.pkl")

                    database_df.to_excel(os.path.join(st.session_state["merged_output_directory"], "proton_binding_dataset.xlsx"))
                        ...
                        with st.spinner("graphing polymers"):
                            ...
                            if i >= 3:
                                ...
                                for polymer in list_of_polymers_by_weight:
                                    if ("NP" not in poly_choice and polymer[0] == grab_polymer_weight(poly_choice)[0] and polymer[1] != grab_polymer_weight(poly_choice)[1] and "NP" not in polymer[0]) or ("NP" in poly_choice and polymer[0] in poly_choice and "NP" not in polymer[0]):
                                        print(polymer[0])
